...
var ParamSets = params.Sets{
	{Name: "Base", Desc: "these are the best params", Sheets: params.Sheets{
		"Network": &params.Sheet{
			...
			// 	}},
			/*{Sel: "#DistanceToHidden", Desc: "top-down back-projections MUST have lower relative weight scale, otherwise network hallucinates",
			Params: params.Params{
				"Prjn.WtScale.Rel": "0.9",
			}},*/
			...
			{Sel: ".InputPrjn", Desc: "Noise for hidden layers",
				Params: params.Params{
					...
					"Prjn.WtInit.Var":  "0.1", // orig 0.15 then 0.25
				}},
...
type Sim struct {
	Size         int               `desc:"size of each dim in 2D input"`
	...
	AxisBool       bool    `which Axis is running`
...
func (ss *Sim) ConfigNet(net *leabra.Network) {
	...
	axis := net.AddLayer2D("Axis", 1, ss.TrainEnv.NAxisUnits, emer.Input)

	face1 := net.AddLayer2D("Face1", 4, 4, emer.Input)
	face2 := net.AddLayer2D("Face2", 4, 4, emer.Input)

	inp1 := net.AddLayer2D("Input1", 6, 6, emer.Hidden)
	inp2 := net.AddLayer2D("Input2", 6, 6, emer.Hidden)
	//hid1 := net.AddLayer2D("Hidden1", 15, 15, emer.Hidden)
	//hid2 := net.AddLayer2D("Hidden2", 15, 15, emer.Hidden)
	combhid := net.AddLayer2D("Hidden", 9, 9, emer.Hidden)
	//hid1 := net.AddLayer2D("Hidden1", 9, 9, emer.Hidden)
	//hid2 := net.AddLayer2D("Hidden2", 9, 9, emer.Hidden)
	//hid2 := net.AddLayer2D("Hidden2", 9, 9, emer.Hidden)
	//dist := net.AddLayer2D("Distance", 1, ss.TrainEnv.NDistUnits, emer.Target)
	output := net.AddLayer2D("Output", 1, ss.TrainEnv.NOutputUnits, emer.Target)
	//x.SetClass("Output")
	//y.SetClass("Output")
	...
	//shid1.SetRelPos(relpos.Rel{Rel: relpos.LeftOf, Other: "Hidden", YAlign: relpos.Front, Space: 2})
	...
	net.BidirConnectLayers(axis, combhid, full)
	...
	net.BidirConnectLayers(combhid, output, full)
...
func (ss *Sim) ApplyInputs(en env.Env) {
	...
	lays := []string{"Output", "Face1", "Face2"}
...
func (ss *Sim) TrialStats(accum bool) {
	...
	if ss.InpTarg {
		if ss.InpLayTarg == 1 { // Inp1 is Target
			...
			inp1tsr := ss.ValsTsr(inp1.Nm)
			inp1.UnitValsTensor(inp1tsr, "ActM")
			inp1Val := ss.TrainEnv.Input1Pop.Decode(inp1tsr.Values)
			...
			inp2tsr := ss.ValsTsr(inp2.Nm)
			inp2.UnitValsTensor(inp2tsr, "ActM")
			inp2Val := ss.TrainEnv.Input2Pop.Decode(inp2tsr.Values)
		...
		ss.TrlCosDiff = float64(output.CosDiff.Cos)
		output_s, output_a := output.MSE(0.5)
		ss.TrlSSE = output_s
		ss.TrlAvgSSE = output_a
	}
...
func (ss *Sim) LogTrnEpc(dt *etable.Table) {
	...
	ss.SumInp1Error = 0
	ss.EpcInp2Error = ss.SumInp2Error / nt
	ss.SumInp2Error = 0
	...
	dt.SetCellFloat("EpcInp2Error", row, ss.EpcInp2Error)
...
func (ss *Sim) ConfigTrnEpcLog(dt *etable.Table) {
	...
	sch := etable.Schema{
		...
		{"EpcInp2Error", etensor.FLOAT64, nil, nil},
	}
	for _, lnm := range ss.LayStatNms {
...
func (ss *Sim) ConfigGui() *gi.Window {
	...
	win := gi.NewMainWindow("exenv", "Leabra Random Associator", width, height)
	ss.Win = win
	...
	tbar.AddAction(gi.ActOpts{Label: "README", Icon: "file-markdown", Tooltip: "Opens your browser on the README file that contains instructions for how to run this model."}, win.This(),
		func(recv, send ki.Ki, sig int64, data interface{}) {
			gi.OpenURL("https://github.com/emer/leabra/blob/master/examples/exenv/README.md")
		})
...
func (ss *Sim) CmdArgs() {
	...
	flag.StringVar(&prnjmean, "prnjmean", "", "prnj wtinit mean")
	flag.StringVar(&prnjvar, "prnjvar", "", "prnj wtinit var")
	flag.Parse()
	ss.Init()
