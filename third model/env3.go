...
func (ev *ExEnv) Config(sz int, ntrls int) {
	...
	ev.Axis.SetShape([]int{ev.NAxisUnits}, nil, []string{"Axis"})
	ev.Output.SetShape([]int{ev.NOutputUnits}, nil, []string{"Output"})
...
func (ev *ExEnv) States() env.Elements {
	els := env.Elements{
		...
		{"Face1", []int{ev.Size}, []string{"Face1"}},
		{"Face2", []int{ev.Size}, []string{"Face2"}},
		{"Input1", []int{ev.Size}, []string{"Input1"}},
		{"Input2", []int{ev.Size}, []string{"Input2"}},
		{"Axis", []int{ev.Size}, []string{"Axis"}},
		{"Output", []int{ev.Size}, []string{"Output"}},
	}
	return els
}
...
func (ev *ExEnv) NewPoint() {
	AxisBool := (rand.Intn(2) == 1) //if true, x axis; if false, y axis
	//choose face out of 16
	input1 := rand.Intn(int(16)) //rand.Intn(int(ev.MaxInp))
	input2 := input1
	for {
		input2 = rand.Intn(int(16))
		if input2 != input1 {
			break
		}
	}
	...
	axisval := 0
	if AxisBool {
		distance = ((input2-1)%4 + 1) - ((input1-1)%4 + 1)
		axisval = 0
	} else {
		distance = (input2 / 4) - (input1 / 4)
		axisval = 1
	}
	out := 0
	if distance < 0 {
	...
	ev.OutputVal = float32(out)
	ev.AxisVal = float32(axisval)
}
